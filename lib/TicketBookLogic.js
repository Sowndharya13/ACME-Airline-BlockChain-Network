 ...
 * @param {org.acme.airline.ticket.BookTicket} TicketBookingData
 * @transaction
 * 
 * **/
function    AssignAircraft(TicketBookingData){
    var ticketRegistry={}
    return getAssetRegistry('org.acme.airline.ticket.Tlight').then(function(registry){
        ticketRegistry = registry
        return ticketRegistry.get(TicketBookingData.ticketId);
    }).then(function(ticket){
        if(!ticket) throw new Error("Ticket : "+TicketBookingData.ticketId," Not Found!!!");
        var   factory = getFactory();
        var   relationship = factory.newRelationship('org.acme.airline.flight','Flight',TicketBookingData.flightId);
        ticket.flight = relationship;
        return ticketRegistry.update(ticket);
    }).then(function(){
        // Successful update
        var event = getFactory().newEvent('org.acme.airline.ticket', 'TicketBooked');
        event.ticketId = TicketBookingData.ticketId;
        event.flightId = TicketBookingData.flightId;
        emit(event);
